...
public class DobbelspelApplicatie {
    ...
    public void startDobbelspelApplicatie(){
        String[] menuAanmelden = {"Stoppen", "Speler aanmelden", "Registreer speler"};
        SpelerDTO aangemeldeSpeler;
        int keuzeMenuAanmelden;
        boolean stoppenMetVragenAanmelden;
        do {
            keuzeMenuAanmelden = kiesUitMenu(menuAanmelden);
            switch (keuzeMenuAanmelden){
                case 1 -> meldAan();
                case 2 -> registreerSpeler();
            }
            aangemeldeSpeler = dc.geefSpeler();
            stoppenMetVragenAanmelden = aangemeldeSpeler != null || keuzeMenuAanmelden == 0;
        }while(!stoppenMetVragenAanmelden);

        if(keuzeMenuAanmelden != 0) {
            String[] menu;
            if (aangemeldeSpeler.adminrechten())
                menu = new String[]{"Stoppen", "Speel spel", "Dien kredietaanvraag in", "Behandel kredietaanvraag"};
            else
                menu = new String[]{"Stoppen", "Speel spel", "Dien kredietaanvraag in"};

            int keuzeMenu = kiesUitMenu(menu);
            while (keuzeMenu != 0) {
                switch (keuzeMenu) {
                    case 1 -> speelSpel();
                    case 2 -> System.out.println("Indienen kredietaanvraag is momenteel nog niet mogelijk");
                    case 3 -> System.out.println("Behandelen kredietaanvraag is momenteel nog niet mogelijk");
                }
                System.out.println();
    ...
    private void meldAan() {
        ...
        if(dc.geefSpeler() == null)
            System.out.println("Aanmelden met deze gegevens is niet mogelijk.");
        else
            System.out.println("Speler is aangemeld!");
    }
